<script>
	import { getAppointment } from '$lib/api/appointment';
...
<div class="flex flex-col items-center space-y-3 mt-10 mx-4">
	...
	<div class="w-full sm:w-3/4 md:w-1/2 lg:w-2/5 xl:w-1/3">
			...
			<div class="flex flex-col sm:flex-row justify-center sm:justify-between items-center space-y-3 sm:space-y-0">
					<button class="border text-white bg-blue-600 py-2 px-3 rounded-md" on:click={() => { goto(`/dashboard`) }}>Back</button>
					<div class="space-x-6">
							<button class="border text-white bg-blue-800 py-2 px-3 rounded-md" on:click={() => { goto(`/new?appointmentId=${appointment._id}`) }}>Edit Appointment</button>
							<button class="border text-white bg-red-600 py-2 px-3 rounded-md" on:click={() => { goto(`/new?appointmentId=${appointment._id}`) }}>Cancel Appointment</button>
					</div>
			</div>
	</div>
</div>
